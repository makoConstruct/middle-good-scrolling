...
We do this in a non-obvious, defter way than more common approaches such as strict axis locking (which feels rigid) or hysteresis (which feels sticky, unresponsive).
...
It can break out into biaxial scrolling when the accumulator turns towards the other axis. Depending on your config, there may be a small jump/pop here to make it clear that this has happened/to recover some of the previously resisted motion.

Before the "axis_decision_threshold" length from drag origin is reached, we still scroll (we don't have a hysteresis pause), but we scroll without committing to one axis or the other. In this pre-decision phase, if you stray from your initial axis to the other, the scroll position will jump over to the other axis.

This gets us immediate responsiveness while still preventing unintentional off-axis movement, and also allowing *intentional* biaxial movement.

### Comparison

Scrollbars are ordinarily pretty bad. The leverage by which movement of the scrollbar handle translates into movement of the page varies a lot depending on the length of the page, sometimes a scroll tab will be a little sliver and so any mouse motion will produce far too much page movement, it will be uncontrollable. When you need to reach for the handle, it will be in a random position, and it will be too narrow. There have been apps that recognized these flaws and hacked up their scrollbar and made it sane, but they're rare. The only one that comes to mind for me is [ripcord](https://cancel.fm/ripcord/), an old discord client (*I would recommend it except for the fact that it seems like discord delete you if you use it, even though third party clients were never mentioned in the ToS and afaik still aren't*) built in Qt. The scrollbar was all handle, and dragging it would always move the page at the rate you'd expect. So rare. Alas.

Mouse wheels? Well, clicky mouse wheels are not great for scrolling. They either move only in small increments (too slow for searching or skimreading), or in large increments (too little control, too jerky), (*[though there is a proposal to add acceleration to scroll wheels](https://github.com/albertz/mouse-scroll-wheel-acceleration-userspace), I'm skeptical as to whether a single stroke of a scrollwheel can send enough scroll events for acceleration to make much of a difference overall, and I'm fairly sure I'll still prefer defter scrolling, so I haven't tried it, if you do try it, feel free to report back.*) There are analog mouse wheels, which are good, but alas, most mouse wheels are not like that, and at this point some applications have come to presume that the mouse wheel is clicky (games use it for item switching).

Touchpads, or mice with touch surfaces? Nah yeah they're great. I think Mac users or laptop users wont have much of an apetite for defter scrolling. I still think defter scrolling is a *little bit* better than those devices, but not by much.

### On dragging and dropping

Ideally, a user should never need to drag and drop things over long distances. Cutting and pasting (*or stowing and popping, or whatever, some other way of moving items*) would always be obviously better. Unfortunately some apps do fail to provide a good alternative to long scroll-drags.  (Examples: *It used to be the case that you couldn't copy and paste a node in roamresearch without breaking transclusions (they've sinced fixed this). Generally, rankings over many items that need to be reordered will be implemented first in a way that doesn't support cut and paste. A remaining example is the manual ordering of the user's wishlist in Steam.*)

There aren't many mouse types that support that. Defter scrolling/scroll-button-drag is a good one. Another one is analogue mouse wheels with low friction free-wheeling, but those are rare and they tend to get gunked up and develop friction after a while. Another approach is often seen in engineers' laptops like Thinkpads or the MNT reforms: trackpads or trackballs or trackpoints with *separate* mouse buttons/keys, so that multitouch gestures aren't needed. Enable middle click scroll and you'd be able to do it.

It could be possible in theory to support long distance drag and drop using regular multitouch trackpad hardware, I'm not sure anything does right now, the usual approach to multifinger gestures would tend to get in the way.